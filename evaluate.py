...
def evaluate_on_val_two_stage(first_stage, generator, critic , data_loader, vgg_for_perceptual_loss, clock, writer = None, results_path = None):
    AdLosses = AvgMeter()
    TvLosses = AvgMeter()
    FakeLosses = AvgMeter()
    ContentLosses = AvgMeter()
...
def test_on_benchmark_two_stage(first_stage, net, save_dir):
    # test model on benchmarks
    # resize ori_img before input it to network
    input_shape = (512, 256)
    ...
    for idx in pbar:
        ...
        with torch.no_grad():
            coarse = first_stage(hole_img.unsqueeze_(0))
            outputs = net(coarse)
