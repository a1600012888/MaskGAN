...
from evaluate import test_on_benchmark_two_stage, evaluate_on_val_two_stage
...
def train(first_stage, generator, critic, optimizer_G, optimizer_C, data_loader,
    ...
    for i, data in enumerate(pbar):
        ...
        coarse = first_stage(inputs)
        outputs = generator(coarse)
        ...
        if clock.minibatch % 200 == 1:
            print('Critic Loss :{:3f}'.format(DLosses.mean))
        if clock.minibatch % iter == 1:
            ...
            p_loss = perceptual_contents[-1] * 5
...
def main():
    ...
    parser.add_argument('--tv', type=float, default=3e-3, help = 'the weight of TV loss(Default:0.003)')
    parser.add_argument('--adv', type = float, default=3.00, help = 'the weight of adv loss(Default:1.0)')

    parser.add_argument('--first_stage', type = str, default='./exps/2_baseline/checkpoint.pth.tar', help = 'first stage model')
    ...
    if args.first_stage != None:
        assert os.path.exists(args.first_stage), 'first stage model does not exist!'
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train(first_stage, generator, critic, optimizer_G, optimizer_C, train_loader,
        ...
        torch.cuda.empty_cache()
        CriticRealLoss, ContentLoss = evaluate_on_val_two_stage(first_stage, generator, critic, valid_loader, vgg_for_perceptual_loss,
